...
namespace CorsaThreading
{
    class Program
    {
        ...
        static void AccettaComandi()
        {
            ...
            switch (choice)
            {
                //per alcune azioni è necessario il lock perchè se il Thread viene sospeso o abortito mentre ha un lock, quest'utlimo non viene rilasciato e tutti gli altri thread sono bloccati.
                case 'S':
                    lock (_lock)
                        thAzione.Suspend();
                    break;
                ...
                case 'R':
                    if (thAzione.ThreadState == ThreadState.Suspended) thAzione.Resume();
                    break;
